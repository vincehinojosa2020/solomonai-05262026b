...
async def generate_deliverable(request: Request):
    """Generate PPTX/DOCX/PDF/XLSX deliverables from Solomon AI content."""
    user = await _get_user_safe(request)
    if not user:
        raise HTTPException(status_code=403, detail="Authentication required")

    body = await request.json()
    deliverable_type = body.get("type", "pdf")
    title = body.get("title", "Solomon AI Report")
    content = body.get("content", "")
    sections = body.get("sections", [])  # [{heading, body}]
    table_data = body.get("table_data")  # {headers: [], rows: [[]]}
    report_id = str(uuid.uuid4())[:8]
    ...
    if deliverable_type == "pdf":
        ...
        elements = [
            ...
            Spacer(1, 6),
            Paragraph(f"Generated by Solomon AI — {datetime.now(timezone.utc).strftime('%B %d, %Y')}", styles["Italic"]),
            Spacer(1, 18),
        ]
        if content:
        ...
        for sec in sections:
            ...
            elements.append(Paragraph(sec.get("heading", ""), styles["Heading2"]))
            elements.append(Spacer(1, 6))
        ...
        if table_data and table_data.get("headers"):
            elements.append(Spacer(1, 12))
            tdata = [table_data["headers"]] + table_data.get("rows", [])
            t = Table(tdata)
        ...
        p2 = tf.add_paragraph()
        p2.text = f"Solomon AI — {datetime.now(timezone.utc).strftime('%B %d, %Y')}"
        p2.font.size = Pt(18)
        ...
        if content:
            ...
            txBox2 = slide2.shapes.add_textbox(Inches(0.8), Inches(0.5), Inches(11.5), Inches(6))
        ...
        for sec in sections:
            ...
            txH = slide_s.shapes.add_textbox(Inches(0.8), Inches(0.3), Inches(11.5), Inches(1))
            pH = txH.text_frame.paragraphs[0]
            pH.text = sec.get("heading", "")
            pH.font.size = Pt(28)
            pH.font.bold = True
            txB = slide_s.shapes.add_textbox(Inches(0.8), Inches(1.5), Inches(11.5), Inches(5))
            txB.text_frame.word_wrap = True
            for j, line in enumerate((sec.get("body", "")).split("\n")):
                if line.strip():
                    pb = txB.text_frame.paragraphs[0] if j == 0 else txB.text_frame.add_paragraph()
                    pb.text = line.strip()
                    pb.font.size = Pt(14)

        buf = _io.BytesIO()
        prs.save(buf)
        filename = f"solomon_{report_id}.pptx"
        ...
        from docx.shared import Pt as DPt, Inches as DInches, RGBColor
        ...
        p_date = doc.add_paragraph(f"Generated by Solomon AI — {datetime.now(timezone.utc).strftime('%B %d, %Y')}")
        p_date.alignment = WD_ALIGN_PARAGRAPH.CENTER
        p_date.runs[0].font.color.rgb = RGBColor(100, 116, 139)
        doc.add_paragraph("")
        ...
        if table_data and table_data.get("headers"):
            doc.add_paragraph("")
            tbl = doc.add_table(rows=1, cols=len(table_data["headers"]))
        ...
        buf = _io.BytesIO()
        doc.save(buf)
        filename = f"solomon_{report_id}.docx"
        return Response(content=buf.getvalue(),
        ...
        if table_data and table_data.get("headers"):
            for col, h in enumerate(table_data["headers"], 1):
                cell = ws.cell(row=1, column=col, value=str(h))
                cell.fill = header_fill
                cell.font = header_font
                cell.alignment = Alignment(horizontal='center')
            for row_idx, row_data in enumerate(table_data.get("rows", []), 2):
                for col_idx, val in enumerate(row_data, 1):
                    ws.cell(row=row_idx, column=col_idx, value=val)
            for col in range(1, len(table_data["headers"]) + 1):
        ...
        buf = _io.BytesIO()
        wb.save(buf)
        filename = f"solomon_{report_id}.xlsx"
        return Response(content=buf.getvalue(),
